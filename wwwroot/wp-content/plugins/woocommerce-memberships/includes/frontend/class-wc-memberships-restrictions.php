...
class WC_Memberships_Restrictions {
		...
		if ( is_singular() && ! is_user_logged_in() ) {
			...
			if ( $restricted ) {
				...
				$redirect_url       = add_query_arg(
					array( 'r' => $post->ID ),
					null === $redirect_permalink ? home_url() : $redirect_permalink
				);
	...
	public function hide_restricted_product_price ( $price, WC_Product $product ) {
		...
		if ( $this->is_restriction_mode( 'hide_content' ) && ! current_user_can( 'wc_memberships_view_restricted_product', $product->id ) ) {
			$price = '';
		}
