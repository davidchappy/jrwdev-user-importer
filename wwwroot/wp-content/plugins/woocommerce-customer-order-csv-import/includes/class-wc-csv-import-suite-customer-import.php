...
class WC_CSV_Import_Suite_Customer_Import extends WC_CSV_Import_Suite_Importer {
	...
	public function update_customer( $id, $data, $options ) {
		...
		try {
			...
			// customer username -- required if wp_insert_user() is used below.
			$user_data['user_login'] = ! empty( $data['username'] ) ? $data['username'] : $customer->user_login;
			...
			if ( ! empty( $user_data ) ) {
				...
				$updated = $send_password_emails ? wp_update_user( $user_data ) : wp_insert_user( $user_data );
