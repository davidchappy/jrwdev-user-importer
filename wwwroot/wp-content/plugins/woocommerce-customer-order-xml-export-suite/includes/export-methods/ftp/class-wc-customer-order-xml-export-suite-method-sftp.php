 ...
 *
 * @package     WC-Customer-Order-XML-Export-Suite/Export-Methods/SFTP
 * @author      SkyVerge
 ...
 * @since 1.1.0
 */
class WC_Customer_Order_XML_Export_Suite_Method_SFTP extends WC_Customer_Order_XML_Export_Suite_Method_File_Transfer {
	 ...
	 *
	 * @since 1.1.0
	 * @see WC_Customer_Order_XML_Export_Suite_Method_File_Transfer::__construct()
	 ...
	 */
	 public function __construct( $args ) {
		...
		if ( ! function_exists( 'ssh2_connect' ) ) {
			...
			throw new SV_WC_Plugin_Exception( __( 'SSH2 Extension is not installed, cannot connect via SFTP.', 'woocommerce-customer-order-xml-export-suite' ) );
		}
		...
		if ( ! $this->ssh_link ) {
			...
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Could not connect via SSH to %1$s on port %2$s, check server address and port.', 'woocommerce-customer-order-xml-export-suite' ), $this->server, $this->port ) );
		}
		...
		if ( ! ssh2_auth_password( $this->ssh_link, $this->username, $this->password ) ) {
			...
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Could not authenticate via SSH with username %s and password. Check username and password.', 'woocommerce-customer-order-xml-export-suite' ), $this->username ) );
		}
		...
		if ( ! $this->sftp_link ) {
			...
			throw new SV_WC_Plugin_Exception( __( 'Could not setup SFTP link', 'woocommerce-customer-order-xml-export-suite' ) );
		}
	}
	...
	public function perform_action( $file_path ) {
		...
		if ( empty( $file_path ) ) {
			throw new SV_WC_Plugin_Exception( __( 'Missing file path', 'woocommerce-customer-order-xml-export-suite' ) );
		}
		...
		if ( ! $stream ) {
			...
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Could not open remote file: %s.', 'woocommerce-customer-order-xml-export-suite' ), $remote_file ) );
		}
		...
		if ( false === fwrite( $stream, $data ) ) {
			...
			/* translators: Placeholders: %s - file name */
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Could not write data to remote file: %s.', 'woocommerce-customer-order-xml-export-suite' ), $filename ) );
		}
	...
	public function handle_errors( $error_no, $error_string, $error_file, $error_line ) {
		...
		throw new SV_WC_Plugin_Exception( sprintf( __( 'SFTP error: %s', 'woocommerce-customer-order-xml-export-suite' ), $error_string ) );
	}
