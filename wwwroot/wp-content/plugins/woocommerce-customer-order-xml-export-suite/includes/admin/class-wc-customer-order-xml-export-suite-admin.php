...
class WC_Customer_Order_XML_Export_Suite_Admin {
	...
	public function __construct() {
		...
		// Add bulk action to download multiple orders to XML and mark them as exported / not-exported
	...
	public function set_settings_page_name() {
		$this->settings_page_name = sanitize_title( __( 'WooCommerce', 'woocommerce' ) ) . '_page_wc_customer_order_xml_export_suite';
	}
	...
	private function get_export_finished_message( $export_id ) {
		...
		$export = wc_customer_order_xml_export_suite()->get_export_handler_instance()->get_export( $export_id );
	...
	private function get_failure_message( $failure_type, $export, $multiple_failures = false ) {
		...
		if ( is_string( $export ) ) {
			$export = wc_customer_order_xml_export_suite()->get_export_handler_instance()->get_export( $export );
		}
		...
		if ( 'export' === $failure_type ) {
			...
			$label = wc_customer_order_xml_export_suite()->get_methods_instance()->get_export_method_label( $export->method );
		...
		?><tr valign="top">
			...
			<td class="forminp forminp-<?php echo sanitize_html_class( $value['type'] ) ?>">
				<input
					...
					data-exclude="wc_customer_order_xml_export_suite_grouped_products"
	...
	public function add_order_status_column_header( $columns ) {
		...
		foreach ( $columns as $column_name => $column_info ) {
			...
			if ( 'order_status' === $column_name ) {
				...
				if ( wc_customer_order_xml_export_suite()->is_plugin_active( 'woocommerce-customer-order-csv-export.php' ) ) {
	...
	public function add_order_status_column_content( $column ) {
		...
		if ( 'xml_export_status' === $column ) {
			...
			if ( $order->wc_customer_order_xml_export_suite_is_exported ) {
	...
	public function add_order_action( $order ) {
		...
		if ( ! get_post_meta( $order->id, '_wc_customer_order_xml_export_suite_is_exported', true ) ) {
		...
		if ( 'shop_order' === $post_type && 'trash' !== $post_status ) {
			...
			<script type="text/javascript">
				jQuery( document ).ready( function ( $ ) {
					var $exported        = $( '<option>' ).val( 'mark_exported_to_xml' ).text( '<?php esc_html_e( 'Mark exported to XML', 'woocommerce-customer-order-xml-export-suite' ); ?>' ),
					    $not_exported    = $( '<option>' ).val( 'mark_not_exported_to_xml' ).text( '<?php esc_html_e( 'Mark not exported to XML', 'woocommerce-customer-order-xml-export-suite' ); ?>' ),
					    $download_to_xml = $( '<option>' ).val( 'download_to_xml' ).text( '<?php esc_html_e( 'Download to XML', 'woocommerce-customer-order-xml-export-suite' ); ?>' );

					$( 'select[name^="action"]' ).append( $exported, $not_exported, $download_to_xml );
					...
					<?php
						...
						if ( $auto_export_method = $this->get_methods_instance()->get_auto_export_method( 'orders' ) ) {
							...
							$export_via_auto_export_method = $( '<option>' ).val( 'export_to_xml_via_auto_export_method' ).text( '<?php echo $label ?>' );
	...
	public function process_order_bulk_actions() {
		...
		if ( 'shop_order' === $typenow ) {
			...
			if ( ! in_array( $action, array( 'download_to_xml', 'mark_exported_to_xml', 'mark_not_exported_to_xml' ), true ) ) {
				return;
			}
			...
			switch ( $action ) {
				...
				case 'mark_not_exported_to_xml':
					...
					foreach( $order_ids as $order_id ) {
						update_post_meta( $order_id, '_wc_customer_order_xml_export_suite_is_exported', 0 );
					}
	...
	public static function get_export_options() {
		...
		$options = array(
			...
			array(
				'id'      => 'type',
				'name'    => __( 'Export Orders or Customers', 'woocommerce-customer-order-xml-export-suite' ),
				'type'    => 'radio',
				'options' => array(
	...
	public function transfer_export() {
		...
		try {
			...
			$this->message_handler->add_error( sprintf( __( 'Could not transfer %1$s %2$s: %3$s', 'woocommerce-customer-order-xml-export-suite' ), $filename, $label, $e->getMessage() ) );
		}
