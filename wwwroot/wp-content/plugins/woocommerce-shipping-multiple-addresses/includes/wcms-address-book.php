    ...
    public function save_addresses() {
        ...
        if (isset($_POST['shipping_address_action']) && $_POST['shipping_address_action'] == 'save' ) {
            ...
            if ( isset($_POST['update_quantities']) || isset($_POST['delete_line']) ) {
                ...
                $next_url = WC()->cart->get_checkout_url();
            }
                                ...
                                case 'phone' :
                                    ...
                                    if ( ! WC_Validation::is_phone( $address[ $key ] ) ) {
                                        ...
                                        if ( function_exists('wc_add_notice') )
                                            ...
                                            WC()->add_error('<strong>' . $field['label'] . '</strong> ' . __( 'is not a valid phone number.', 'woocommerce' ));
                                    }
                                ...
                                case 'email' :
                                    ...
                                    if ( ! is_email( $address[ $key ] ) ) {
                                        ...
                                        if ( function_exists('wc_add_notice') )
                                            ...
                                            WC()->add_error( '<strong>' . $field['label'] . '</strong> ' . __( 'is not a valid email address.', 'woocommerce' ) );
                                    }
                                    ...
                                    if ( is_array($valid_states) && sizeof( $valid_states ) > 0 )
                                        if ( ! in_array( $address[ $key ], array_keys( $valid_states ) ) ) {
                                            ...
                                            if ( function_exists('wc_add_notice') )
                                                ...
                                                WC()->add_error('<strong>' . $field['label'] . '</strong> ' . __( 'is not valid. Please enter one of the following:', 'woocommerce' ) . ' ' . implode( ', ', $valid_states ));
                                        }
