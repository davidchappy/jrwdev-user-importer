            ...
            if (!isset($_GET['order_id']) || empty($_GET['order_id'])) {
                ...
                foreach ( $packages as $x => $package ) {
                    ...
                    foreach ( $products as $i => $product ) {
                        ...
                        echo '<td class="product-name"><a href="'. get_permalink($product['data']->id) .'">'. apply_filters( 'wcms_product_title', get_the_title($product['data']->id), $product ) .'</a><br />'. $item_meta .'</td>';
                ...
                <td style="vertical-align: top;" colspan="<?php if ( version_compare( WOOCOMMERCE_VERSION, '2.0', '<' ) ) echo '2'; else echo '1'; ?>">
                    ...
                    <div id="shipping_addresses">
                        ...
                        foreach ($packages as $x => $package):
                            ...
                            if (! $has_address ) {
                                ...
                                <div class="ship_address no_shipping_address">
                                    ...
                                    <?php
                                        foreach ($products as $i => $product):
                                            $attributes = WC()->cart->get_item_data($product);
                                            ?>
                                            <li>
                                                <strong><?php echo apply_filters( 'wcms_product_title', get_the_title($product['data']->id), $product ); ?> x <?php echo $product['quantity']; ?></strong>
                                                <?php
                                                if ( !empty( $attributes ) ) {
                        ...
                        <div class="ship_address">
                            ...
                            <?php
                                foreach ($products as $i => $product):
                                    $attributes = WC()->cart->get_item_data( $product, true );
                            ?>
                            <dd>
                                <strong><?php echo apply_filters( 'wcms_product_title', get_the_title($product['data']->id), $product ); ?> x <?php echo $product['quantity']; ?></strong>
                                <?php
            ...
            if ( is_null($sess_cart_addresses) || empty($sess_cart_addresses) ) {
                ...
                if (wcms_count_real_cart_items()>0) foreach (wcms_get_real_cart_items() as $cart_item_key => $values) {
                    ...
                    for ($i = 1; $i <= $qty; $i++) {
                        ...
                        $meta                           = md5(WC()->cart->get_item_data($_value));
